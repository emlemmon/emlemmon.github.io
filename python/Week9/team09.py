...
class CheckingAccount:
    def __init__(self, starting_balance, num_checks):
        ...
        if self.balance <= 0:
            raise BalanceError("BalanceError: Low balance")
    ...
    def write_check(self, amount):
        ...
        if self.check_count <= 0:
            raise OutOfChecksError("OutOfChecksError: Get more Checks")
    ...
    def display(self):
        print(f"${self.balance:.2f} -- {self.check_count}")

    def apply_for_credit(self, amount):
        pass

def get_more_checks(account):
    more_checks = input("Would you like to buy more checks for $5? (yes/no): ")
    if more_checks == "yes":
        account.balance -= 5
